...
if (navigator.mediaDevices.getUserMedia) {
    ...
    let onSuccess = function(stream) {
        ...
        mediaRecorder.onstop = function(e) {
            const clipName = prompt('Please enter a name for your sound clip:','My-Audio');
            ...
            clipLabel.onclick = function() {
                ...
                const newClipName = prompt('Please enter a name for your sound clip:', existingName);
                if(newClipName === null) {
    ...
    let onError = function(err) {
        alert("Live recordings is not supported by your browser! Please try the latest browser");
    }   
...
async function upload(formData){
    let response = await fetch('/upload/audio', {
      ...
      if(response.status == 200){
        ...
        alert('Your file is not uploaded');
        window.close();
      }
}
